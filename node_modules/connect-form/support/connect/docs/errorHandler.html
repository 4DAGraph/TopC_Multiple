...
<p>The <em>errorHandler</em> middleware responds to exceptions passed or caught from within middleware above it, according to the options shown below.</p>
...
<pre><code class="js"><span class="variable">connect</span>.<span class="variable">createServer</span>(
    <span class="keyword">function</span>(<span class="variable">req</span>, <span class="variable">res</span>, <span class="variable">next</span>){
        <span class="keyword">throw</span> <span class="keyword">new</span> <span class="class">Error</span>(<span class="string">'ahhhh!'</span>);
    },
    <span class="variable">connect</span>.<span class="variable">errorHandler</span>({ <span class="variable">dumpExceptions</span>: <span class="variable">true</span> })
);
</code></pre>

<p>By default the response of <em>errorHandler</em> is 500 'Internal Server Error'. If <em>showStack</em> is true, then Connect decides how to respond based on the <em>Accept</em> request header:</p>

<pre><code>text/html         Outputs an html version of the stack trace
...
<pre><code>showMessage     Show the exception message only  
