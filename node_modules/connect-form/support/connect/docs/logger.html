...
<p>The <em>logger</em> middleware provides common log format support, as well as custom formats, below is the default usage:</p>
...
<p>A custom format can also be passed:</p>

<pre><code class="js"><span class="variable">connect</span>.<span class="variable">createServer</span>(
    <span class="variable">connect</span>.<span class="variable">logger</span>({ <span class="variable">format</span>: <span class="string">':method :url :response-time'</span> })
);
</code></pre>

<p>The following format tokens are currently available:</p>

<pre><code>:req[header]    Arbitrary request header, ex: req[Accept]
